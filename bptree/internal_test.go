...
func Test_internalNode_insert(t *testing.T) {
	...
	n := newInternalNode(nil, 3)
	assert.Equal(n.kcs.Len(), 3)
	p1 := newLeafNode(nil, 2)
	p1.insert(1, "c")
	p1.insert(5, "a")
	n.insert(0, p1) // 0表示无

	p2 := newLeafNode(nil, 2)
	p2.insert(12, "b")
	n.insert(5, p2)
...
func Test_internalNode_split(t *testing.T) {
	...
	p1 := newLeafNode(nil, 3)
	p1.insert(1, "1")
	n.insert(0, p1) // 0表示无

	p2 := newLeafNode(nil, 3)
	p2.insert(5, "5")
	n.insert(5, p2)
	...
	p3 := newLeafNode(nil, 3)
	p3.insert(12, "12")
	n.insert(12, p3)

	p4 := newLeafNode(nil, 3)
	p4.insert(18, "18")
	p4.insert(21, "21")
	n.insert(18, p4)
}
...
func Test_internalNode_remove(t *testing.T) {
	...
	child := newLeafNode(nil, 3)
	child.insert(5, "a")
	child.insert(12, "b")
	child.insert(1, "c")
	child2 := newLeafNode(nil, 3)
	child3 := newLeafNode(nil, 3)
